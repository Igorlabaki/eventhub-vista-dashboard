...
export function DocumentList({
  ...
  const handleDelete = async (documentId: string) => {
    try {
      ...
      const { title, message } = handleBackendSuccess(response, "Documento excluído com sucesso!");
      showSuccessToast({
        title,
        description: message
      });
      ...
      const { title, message } = handleBackendError(error, "Erro ao excluir documento. Tente novamente mais tarde.");
      toast({
        title,
        description: message,
        variant: "destructive",
  ...
  if (!documents || documents.length === 0) {
    return <EmptyState title={emptyMessage} actionText="Novo Documento" onAction={onCreateClick} />;
  }
  ...
  return (
    <div className={cn("space-y-4", className)}>
      ...
      <ConfirmDeleteDialog
        ...
        entityName={documentToDelete ? `Documento "${documentToDelete.title}"` : ""}
