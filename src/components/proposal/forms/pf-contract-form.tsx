...
export default function PFContractForm() {
    ...
    const onSubmit = async (values) => {
        try {
          ...
          const { title, message } = handleBackendSuccess(response, "Dados pessoais atualizados com sucesso!");
          showSuccessToast({
            title,
            description: message
          });
        } catch (error: unknown) {
          const { title, message } = handleBackendError(error, "Erro ao atualizar dados pessoais. Tente novamente mais tarde.");
          toast({
            title,
            description: message,
            variant: "destructive",
    ...
    // Atualiza o valor do sinal automaticamente ao mudar o número de parcelas
    React.useEffect(() => {
      const numberPayments = Number(form.watch('paymentInfo.numberPayments'));
              ...
              <div className="flex gap-4">
                ...
                <button
                  ...
                  onClick={() => generateContractPdf({
                    contract: venueContracts.find(contract => contract.id === form.getValues().contractId),
                    owner: ownersByVenueId.find(owner => owner.id === form.getValues().ownerId),
                    currentProposal,
                    selectedVenue,
