...
export default function VenueBudgets() {
  ...
  const filteredBudgets = mockBudgets.filter(budget => budget.clientName.toLowerCase().includes(searchTerm.toLowerCase()) || budget.eventType.toLowerCase().includes(searchTerm.toLowerCase()));
  ...
  return <DashboardLayout title="Orçamentos" subtitle="Gerencie seus orçamentos">
      <div className="mb-6 flex items-center justify-between">
        <div className="relative w-full max-w-md">
          <Search className="absolute left-2.5 top-2.5 h-4 w-4 text-gray-500" />
          <Input placeholder="Buscar orçamentos..." className="pl-9" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
        </div>
      ...
      <div className="rounded-md border bg-white">
        <Table>
          ...
          <TableBody>
            {filteredBudgets.length > 0 ? filteredBudgets.map(budget => <TableRow key={budget.id} className="cursor-pointer hover:bg-gray-50" onClick={() => handleOpenBudgetDetails(budget)}>
